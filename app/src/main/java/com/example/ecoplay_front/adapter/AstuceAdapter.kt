...
import com.example.ecoplay_front.view.AstuceDetailsActivity
...
class AstuceAdapter : RecyclerView.Adapter<AstuceAdapter.AstuceViewHolder>() {
    ...
    inner class AstuceViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(astuce: Astuce) {
            ...
            val imagePrefix = "http://192.168.128.65:8088/images/astuce/"
            val imageUrl = imagePrefix + astuce.imageDetailA
            Glide.with(itemView.context)
                .load(imageUrl)
                .into(imageView)
            val imageDetail = imagePrefix + astuce.imageItemA
            Glide.with(itemView.context)
                .load(imageDetail)
                .into(imageView)
